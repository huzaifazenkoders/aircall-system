...
const LeadDetailsModalContent = ({ mode }: { mode: "modal" | "page" }) => {
    ...
    <div className="overflow-hidden rounded-2xl">
      <div className="flex flex-wrap items-center justify-between gap-6 border-b border-border px-8 py-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button variant="outline" className="h-11 rounded-xl px-5 text-sm font-medium">
      ...
      <div className={cn("grid gap-10 px-8 py-8", mode === "modal" ? "lg:grid-cols-[1fr_320px]" : "lg:grid-cols-[1fr_360px]")}>
        <div className="min-w-0">
          ...
          <div className="mt-12 overflow-hidden rounded-2xl bg-muted/30 ring-1 ring-border">
            <div className="flex items-center justify-between gap-4 border-b border-border px-6 py-5">
              <div className="text-lg font-semibold tracking-tight">Purchase History</div>
                ...
                <TableBody>
                  {Array.from({ length: 7 }).map((_, i) => (
                    <TableRow key={i} className="h-14">
                      <TableCell className="font-medium text-foreground">VIP Ticket</TableCell>
                      <TableCell className="font-semibold text-foreground">$997</TableCell>
                      <TableCell className="text-foreground">06/12/26</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
        ...
        <div className="min-w-0">
          <Tabs defaultValue="history" className="w-full">
            <TabsList className="h-auto w-full justify-start gap-8 bg-transparent p-0">
              <TabsTrigger value="history" className="px-0 pb-4 text-sm font-semibold">
                Call History
              </TabsTrigger>
              <TabsTrigger value="notes" className="px-0 pb-4 text-sm font-semibold text-muted-foreground">
                Keap Notes
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="history" className="m-0 pt-6">
              <div className="relative pl-8">
                <div className="absolute top-0 bottom-0 left-3.5 w-px bg-border" aria-hidden="true" />
                <div className="flex flex-col gap-8">
                  <HistoryItem title="No Answer" subtitle="Olivia Smith on Feb 20, 2025 at 10:12 AM" />
                  <HistoryItem title="No Answer" subtitle="Olivia Smith on Feb 20, 2025 at 10:12 AM" />
                  <HistoryItem title="No Answer" subtitle="Olivia Smith on Feb 20, 2025 at 10:12 AM" />
                  <HistoryItem title="Voice Mail" subtitle="Olivia Smith on Feb 20, 2025 at 10:12 AM" />
                </div>
              </div>
            </TabsContent>
...
const SectionTitle = ({ children }: { children: React.ReactNode }) => {
  return <div className="text-xs font-semibold tracking-wider text-foreground">{children}</div>;
};
...
const HistoryItem = ({ title, subtitle }: { title: string; subtitle: string }) => {
  return (
...
export default LeadDetailsModalContent;
